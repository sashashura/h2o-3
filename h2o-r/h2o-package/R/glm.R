...
#'        upper indices in the approximating of the infinite series summation. Defaults to 8e-27.
...
#'        Defaults to TRUE.
#' @param logll_length only valid for dispersion_parameter_model = ml, family = tweedie, fix_dispersion_parameter=false,
...
#'        the array before we stop the process.  In general, the longer the list, the better dispersion value we will
#'        get.  However, this will increase the run time.  Default to 8. Defaults to 8.
                    ...
                    dispersion_learning_rate = 0.5)
{
  ...
  if (!missing(logll_length))
    parms$logll_length <- logll_length
