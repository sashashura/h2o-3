...
import static hex.glm.DispersonTask.ConstColNames.*;
import static hex.glm.DispersonTask.InfoColNames.*;
...
public class DispersonTask {
    ...
    public static class ComputeMaxSumSeriesTsk extends MRTask<ComputeMaxSumSeriesTsk> {
        ...
        public ComputeMaxSumSeriesTsk(TweedieMLDispersionOnly tdispersion, GLMModel.GLMParameters parms) {
            ...
            for (int rInd = 0; rInd < chkLen; rInd++) {
                ...
                if (response > 0) { // this part is only valid for response > 0
                    ...
                    if (Double.isFinite(logZ)) {
                        ...
                        sumWVj = sumWV(jKL, jKU, wvMax, logZ, new EvalLogWVEnv(), 0);
                        if (sumWVj <= 0.0)
                            _nobNegSum++;
                        oneOverSumWVj = 1.0 / Math.abs(sumWVj);
                        sumDWVj = sumWV(djKL, djKU, dwvMax, logZ, new EvalLogDWVEnv(), 1) * _alphaMinus1OverPhi;
                        sumD2WVj = sumWV(d2jKL, d2jKU, d2wvMax, logZ, new EvalLogD2WVEnv(), 2) * _alphaMinus1SquareOverPhiSquare
                ...
                if (_debugOn) {
                    ...
                    setDebugValues(rInd, jKIndMax, logZ, wvMax, dwvMax, d2wvMax, jKL, jKU, djKL, djKU, d2jKL, d2jKU,
                            sumWVj, sumDWVj, sumD2WVj, tempLL, tempDLL, tempD2LL, chks, infoColName2Ind);
                }
            }
                Log.info("Chunk IDX "+chks[0].cidx()+" contains "+_nobNegSum+" rows of data with series sum < 0.");
        }
        ...
        public void setDebugValues(int rInd, int jkIndMax, double logZ, double wvMax, double dwvMax, double d2wvMax,
                                   ...
                                   Map<InfoColNames, Integer> infoColName2Ind) {
        ...
        double sumWV(int jkL, int jkU, double logWVMax, double logZ, CalWVdWVd2WV cCal, int wvIndex) {
            if (_variancePower < 2) {
                ...
                for (int index=jkL; index<=jkU; index++)
                    seriesSum2 += Math.exp(cCal.calculate(index,
                            _alpha, logZ, logWVMax, _variancePower) - logWVMax) * Math.pow(-1, index) * Math.sin(-index * _alphaTimesPI);
...
/*                if (seriesSum < 0.0 && wvIndex == 0) {  // avoid series sum to negative for w
                    for (int index = jkU+1; index < _indexBound; index++) {
                        seriesSum +=  Math.exp(cCal.calculate(index, _alpha, logZ, logWVMax, _variancePower) - 
                                logWVMax) * Math.pow(-1, index) * Math.sin(-index * _alphaTimesPI);
                        seriesSum2 += Math.exp(cCal.calculate(index, _alpha, logZ, logWVMax, _variancePower) -
                                logWVMax) * Math.pow(-1, index) * Math.sin(-index * _alphaTimesPI);
                        if (seriesSum >= 0.0)
                            break;
                    }
